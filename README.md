...
`LimbusEGO-1.4.1.jar` 是由兩個舊插件合併而成的單一插件：
...
2. **放入新插件**：把 `LimbusEGO-1.4.1.jar` 放進 `plugins/`。
...
### 1.4.1（2026-07-07）— 飾品升級綁定物品本身

- **升級改綁定飾品物品**：升級等級寫入飾品 ItemStack PDC（`item_upgrade_level`），拖出選單、轉交他人、存箱子等級都跟著物品走；背包中同 ID 未升級飾品不繼承等級
- **Lore 隨升級變動**：等級 > 0 的飾品描述末尾顯示「▲ 升級 Lv.N (效果 ×M)」（新 lang 鍵 `msg.upgrade_lore_mult`，舊語言檔自動回退 jar 內建值）
- **裝備格等級持久化**：玩家 PDC 新增 `slot_<i>_level`，關選單時從物品讀等級存入、開選單時烙回重造物品；效果倍率改讀裝備格等級（同 ID 裝多格取最大），40+ 飾品效果檔零改動
- **舊資料一次性遷移**：登入時將舊玩家 PDC `upgrade_<id>` 轉寫到目前裝備格並清除舊鍵；已升級但未裝備的 ID 等級作廢

### 1.4.0（2026-07-07）— 狀態系統重做：原版傷害管線、上限與重平衡

- **狀態傷害改走原版傷害管線**：護甲、附魔、抗性藥水、領地/PvP 保護全部自動生效；DoT 擊殺正確歸屬施術者。修復禁 PvP 領地內可疊 DoT 掛人的漏洞（傷害監聽加 `ignoreCancelled` 與零傷閘）
- **層數/級數上限**：potency/count 疊加各設上限（config `status-caps`，預設 10/20）；守護（PROTECTION）與易損（FRAGILE）改為受擊消耗層數，減傷數學上限 40%，杜絕掛機無敵
- **20HP 基準重平衡**：狀態係數以「未穿甲玩家中等致命度」重排（燒傷/流血/沉淪 ×0.5、破裂 ×0.75、震顫爆發 ×1.5、強壯/易損 5%、守護 4%、充能 3%、呼吸法爆率 5%/上限 50%/爆傷 ×1.5）
- **狀態施加冷卻**：同效果對同目標的重複施加有最短間隔（config `status-cooldowns`，預設 1000ms）
- **PlaceholderAPI 支援**：`%limbusego_san%`、`%limbusego_effects%`、`%limbusego_<效果>_potency|count%`（軟依賴，未裝不影響）
- **新增 `/limbusego status apply|get` 管理指令**：控制台/管理員施加與查詢狀態，支援目標選擇器，含 Tab 補完
- 修復：無來源 DoT 使用會穿甲的 `generic` 傷害型別，改用 `MOB_ATTACK` 讓護甲正確生效（拋棄式伺服器實測驗證，與原版護甲公式吻合）
- 文件：新增狀態重平衡設計文件與罪孽屬性抗性系統企劃書（`docs/plans/`）
