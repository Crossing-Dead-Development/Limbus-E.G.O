...
public class solemnlament {
    ...
    public solemnlament(LimbusEGO plugin) {
        this.plugin = plugin;
                    ...
                    if (isBlack) {
                        ...
                        if (plugin.getStatusManager() != null) {
                            plugin.getStatusManager().apply(target,
                                    me.yisang.limbusego.status.StatusEffect.SINKING, 4, 3, player);
                        }
                    } else {
                        ...
                        if (plugin.getStatusManager() != null) {
                            plugin.getStatusManager().apply(target,
                                    me.yisang.limbusego.status.StatusEffect.SINKING, 3, 2, player);
                        }
                    }
    ...
    // ── 聖宣盾牌 Tick 邏輯（由 LimbusEGO.startShieldTick 每 5 tick 呼叫）──
    ...
    public void handleShieldTick(Player player, ItemStack item) {
        ...
        // 自身守護：上限 3 potency
        me.yisang.limbusego.status.StatusManager sm = plugin.getStatusManager();
        if (sm != null) {
            me.yisang.limbusego.status.StatusState s = sm.get(player);
            int cur = s == null ? 0 : s.potency(me.yisang.limbusego.status.StatusEffect.PROTECTION);
            if (cur < 3) {
                sm.apply(player, me.yisang.limbusego.status.StatusEffect.PROTECTION, 1, 40, player);
            }
        }
        ...
        player.getNearbyEntities(5, 5, 5).forEach(e -> {
            if (e instanceof LivingEntity target && !e.equals(player)) {
                ...
                if (sm != null) {
                    sm.apply(target, me.yisang.limbusego.status.StatusEffect.BIND, 1, 2, player);
                }
            }
        });
    }
